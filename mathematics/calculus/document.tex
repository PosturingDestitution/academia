...
\textbf{scalars and vectors}\\
	Scalars have only magnitude (speed, temperature, mass). A vector has both magnitude and direction (velocity , force, displacement).\\
	the magnitude (always positive because scalar) $\lvert\vec{a}\rvert = 10$ and $\theta = 30^{\circ}$ you can find the components as such: $10\cos(30^{\circ})$ and $10\sin(30^{\circ})$ thus (8.66, 5). if we were given $\vec{a} = (8.66, 5)$ we know that both its $x$ and $y$ component are in $Q1$ $\theta = \tan^{-1}(\frac{y}{x})$.\\

	$\vec{w} = (1, 2)$ what is $-2\vec{w} = (-2, -4)$ (note that graphically multiplying by -1 flips the vector also note  $i^2 = -1$ for later\\
	graph $\vec{a} = (3, -1)$ and $\vec{b} = (2, 3)$ find the sum and difference and observe the graph $\vec{a} + \vec{b}$ and $\vec{a} - \vec{b}$ ($\vec{a} + (-\vec{b})$) and graph
